<?php  
        ...
        include_once ('../../../controller/controlequipo.php');
        include_once ('../../../model/Equipos.php');
        include_once ('../../../controller/controlequiporeserva.php');
        include_once ('../../../model/EquipoReserva.php');
        $id = $_POST["btn"];
  ...
  <table id='keywords' cellspacing="0" cellpadding="0">
    ...
    <tbody>     
      <tr>
       ...
       <?php $idvh = $vehiculo->getId_vh();
            $reservas = ReservasControl::traerreserva(Conexion::obtener_conexion(), $idvh);
            if (count($reservas)) {
    ...
    <tbody>     
      <tr>
        <td><?php echo $reserva->getValor_res(); ?></td>
         ...
         <td><form action="localizarvehiculo.php" method="post">
         <button class="btn" name="btn" value="<?php echo $vehiculo->getId_vh(); ?>"> Localizar <span class="arrow">❯</span></button>
        </form></td> 
      </tr>
    </tbody>
  </table>
...
$id_res=$reserva->getId_res();
  ...
  $equiposred = equiposreserva::equipores(Conexion::obtener_conexion(), $id_res);
              if (count($equiposred)) {
           foreach ($equiposred as $equiposres) {
 ...
 <div id="wrapper">
  ...
  <table id='keywords' cellspacing="0" cellpadding="0">
    ...
    <tbody>
      ...
      <tr>   <?php
  $id_eres= $equiposres->getId_equipoReserva();
  $equipos = equipo::equipos(Conexion::obtener_conexion(), $id_eres);
              if (count($equipos)) {
           foreach ($equipos as $equipo) {
                  
                 ?>  
        <td><?php echo $equipo->getTalla_eq(); ?></td>
        <td><?php echo $equipo->getColor_eq(); ?></td>
        <td><?php echo $equipo->getDescripcion(); ?></td>
        <td><?php echo $equiposres->getCantidad(); ?></td>
      <?php } } ?>
      </tr> 
    </tbody>
  </table>

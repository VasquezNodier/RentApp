...
<?php
 ...
 $res = mysqli_query($conexion, "SELECT id_pq, nombre_pq FROM parqueaderos");
            ...
            <?php } ?>
						...
						<li><a class="nav-link scroll-link" href="mantenimiento.php">Mantenimiento</a></li>
					 </ul>
 ...
 $res = mysqli_query($conexion, "SELECT * FROM vehiculos where  parqueadero='$id' order by estado_vh");
                while($usuario = mysqli_fetch_array($res)){
                  
                 ?>
      <tr>
        <td class="lalign"><?php echo $usuario['modelo_vh']; ?></td>
       
        <td><?php echo $usuario['color_vh']; ?></td>
        <td><?php echo $usuario['placa_vh']; ?></td>
        <td><?php echo $usuario['estado_vh']; ?></td>
         <td><form action="vervehiculo.php" method="post">
         <button class="btn" name="btn" value="<?php echo $usuario['id_vh']; ?>"> Ver <span class="arrow">❯</span></button>
        </form></td>
        <td></td>
      </tr> <?php } ?>
    </tbody>
